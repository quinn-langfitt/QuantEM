...
// MODIFICATIONS:
// - Decoupled permitted gates and set of candidate SWAPs
...
use qiskit_accelerate::sabre::sabre_dag::SabreDAG;
use qiskit_accelerate::sabre::swap_map::SwapMap;
use qiskit_accelerate::sabre::{BlockResult, NodeBlockResults, SabreResult};

// No changes made here: just b/c Qiskit didn't make these public
...
impl RoutingState<'_, '_> {
    ...
    fn force_enable_closest_node(
        ...
        let neighbor_graph: Graph<(), ()> = self.target.neighbors.coupling_graph();
        ...
        let shortest_path = {
            ...
            (dijkstra(
                ...
                |e| match neighbor_graph.find_edge(e.source(), e.target()) {
                    Some(_) => Ok(1.0),
                    None => Ok(999999.0),
                },
        ...
        if shortest_path.len() <= 2 {
            println!(
                "this should have already been routed...: {:?}",
                shortest_path
            );
            return smallvec![closest_node];
        }
        ...
        if !neighbor_graph
            .node_indices()
            .contains(&NodeIndex::new(shortest_path[0].index()))
        {
            for i in 0..shortest_path.len() - 2 {
                let end = shortest_path.len() - 1 - i;
                let swap = [shortest_path[end], shortest_path[end - 1]];

                // println!("    swapR: {:?}", swap);
                current_swaps.push(swap);
            }
        } else if !neighbor_graph.node_indices().contains(&NodeIndex::new(
            shortest_path[shortest_path.len() - 1].index(),
        )) {
            for i in 0..shortest_path.len() - 2 {
                let swap = [shortest_path[i], shortest_path[i + 1]];
...
#[pyo3(signature=(dag, neighbor_table, coupling_map, distance_matrix, heuristic, initial_layout, num_trials, seed=None, run_in_parallel=None))]
pub fn sabre_routing(
    py: Python,
    dag: &SabreDAG,
    neighbor_table: &NeighborTable, // Set of SWAP candidates
    coupling_map: &NeighborTable,   // Gate connectivity
    ...
    let target = RoutingTargetView {
        ...
        coupling: &coupling_map.coupling_graph(),
