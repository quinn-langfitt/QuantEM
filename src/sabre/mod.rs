...
//
// MODIFICATIONS (2025)
// - Modified module to point to qiskit_accelerate when appropriate and this modified routing module when appropriate.

pub mod route;

// Qiskit made a bunch of stuff private
pub mod heuristic;
pub mod layer;
...
pub fn sabre(m: &Bound<PyModule>) -> PyResult<()> {
    ...
    m.add_class::<qiskit_accelerate::sabre::sabre_dag::SabreDAG>()?;
    // m.add_class::<SwapMap>()?;
    // m.add_class::<BlockResult>()?;
    // m.add_class::<NodeBlockResults>()?;
    // m.add_class::<SabreResult>()?;

    m.add_class::<qiskit_accelerate::nlayout::NLayout>()?;
    Ok(())
}
